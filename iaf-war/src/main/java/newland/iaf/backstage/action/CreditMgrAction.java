...
@Namespace("/backstage")
@Action(value = "creditMgr")
@Results({
	@Result(name = "list", type = "JqgridJsonResult"),
	@Result(name = "index",  type = "dispatcher",location = "/backstage/creditmgr/index.jsp"),
	@Result(name = "viewLoanOrd",  type = "dispatcher",location = "/backstage/creditmgr/loanOrdDetail.jsp")
    })
public class CreditMgrAction extends BSBaseAction {
	...
	public void query(Page page) throws Exception{
		...
		//cond.setType(FundFlowType.CREDIT);
		//cond.setStat(FundFlowStat.AUDIT);
	...
	static {
		types.add(FundFlowType.CREDIT);
		types.add(FundFlowType.OTH_CREDIT);
	}
	...
	public String viewFundFlowDetail(){
		...
		try{
			this.loanFundFlowList = this.backStageLoanService.queryFundFlowBy(types,loanOrd.getIloanOrd());
	...
	@JqDataSet(content = "{o:traceNo},{o:loanOrdId},{o:merchName},{o:instName},{i:(format.time)({o:genTime})},{f:(wanyuanFormatter)({o:amount})},{o:type}")
