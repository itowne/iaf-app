...
<div id="workframe-wrapper" class="clearfix">
  ...
  <form action="/inst/loanord/refundMgr!" onsubmit="return check();">
  <div align="center" style="font-size:16px;font-weight: bold">解冻申请已登记！</div>
  <div align="center" style="font-size:16px;font-weight: bold ;color:red;margin-top: 10px">请下载打印以下的解冻申请凭证单，并盖上贵公司的印章后，扫描成彩色图片上传到汇融易平台</div>
  <div id="content" class="report" align="center" style="margin-top: 20px">
    ...
    <tr>
    	...
    	<td class="data" colspan="3" style="text-align:left">${loanOrd.unFreezeMemo}</td>
    </tr>
    </table>
    <input type="hidden" name="selectedRows" id="selectedRows" value="<s:property value="selectedRows"/>">
     <input type="hidden" name="iloanOrd" id="iloanOrd" value="<s:property value="loanOrd.iloanOrd"/>">
      <div align="center" style="margin-top: 10px"><input class="dark-btn" type="button" value="下载解冻申请单" onclick="print()">&nbsp;&nbsp;&nbsp;<input type="button" class="dark-btn" value="稍后再打印" onclick="cancel()"/></div>
  </div>
  </form>
<script type="text/javascript">
$("#myflow").flow(["填写解冻申请","下载打印解冻申请凭证，并加盖公司印章","上传申请凭证的扫描图片"],2);
	$(function(){
	...
	function print(){
		...
		if(confirm("确认要打印凭证?")){
			window.location.href="${ctx}/inst/loanord/refundMgr!unFreezeprint?selectedRows="+selectedRows;			
		}
	}
</script> 
</body>
