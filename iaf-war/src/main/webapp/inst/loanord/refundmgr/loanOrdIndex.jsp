	...
	alert("<s:property escape="false"/>") 
		...
		<div id="workframe-wrapper" class="clearfix">
			...
			<div id="content">
				...
				<s:form action="refundMgr">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
									<td>
										...
										<select id="termType" name="termType" style="width:120px;" class="validate[required]">
											...
											<tr>
												...
												<td class="data">
													<s:select list="#request.status" listKey="name()" listValue="desc" id="ordStat" headerKey="" headerValue="请选择" cssClass="data" name="ordStat"></s:select>
												</td>
											</tr>
										</table>
									</td>
									<td class="search-btn">
										&nbsp;&nbsp;
										<input type="hidden" name="ten" id="ten" value="<s:property value="ten"/>"/>
										<input type="hidden" name="freeze" id="freeze" value="<s:property value="freeze"/>">
										<!--<s:submit cssClass="dark-btn" method="loanQuery" value="查    询"></s:submit> -->
				...
				<h3 class="title"><span>查询结果</span>&nbsp;&nbsp;&nbsp;<!--  <a href="${ctx}/inst/loanord/refundMgr!beginQueryhis" class="dark-btn">查询已付清订单</a>--></h3>
		...
		<script type="text/javascript">
			...
			function getList(){
				...
				jQuery("#list").jqGrid({
					url:'${ctx}/inst/loanord/refundMgr!list?ten='+ten+'&freeze='+freeze,
					colNames:['借贷编号','申请类型','贷款产品','贷款金额','贷款周期','贷款利率','申请商户','最近一期还款时间','订单状态','操作'],
					colModel:[
						...
						{name:'merchName',index:'merchName',align:"center", width:80},
						//{name:'merchType',index:'merchType',align:"center", width:50},
						{name:'last',index:'last',align:"center", width:65},
			...
			function statFormatter(cellvalue, options, rowObject){
			 ...
			 function storeQuery(){
				 var loanOrdId=$("#loanOrdId").val();
					...
					$("#list").jqGrid('setGridParam',{url:'${ctx}/inst/loanord/refundMgr!list?loanOrdId='+loanOrdId+'&pdtType='+pdtType+'&minloanAmount='+minloanAmount+'&maxloanAmount='+maxloanAmount+'&minTerm='+minTerm+'&maxTerm='+maxTerm+'&merchName='+encodeURI(merchName)+'&endDate='+endDate+'&acceptDate='+acceptDate+'&ordStat='+ordStat+'&termType='+termType+'&rateType='+rateType+'&minRate='+minRate+'&maxRate='+maxRate+'&pdtId='+pdtId+'&minDate='+minDate+'&maxDate='+maxDate,page:1}).trigger("reloadGrid");
			 }
		</script>
