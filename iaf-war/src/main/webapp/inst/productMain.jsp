	...
	<body>
		<div id="workframe-wrapper" class="clearfix">
			...
			<div id="content">
				...
				<div style="float:right;padding: 0"><n:instAuthButton value="发布新产品" cssClass="dark-btn" onclick="productAdd();" authCode="WDCP_XG"/></div>
		...
		<script type="text/javascript">
			...
			function getList(){
				jQuery("#list").jqGrid({
					url:'${ctx}/inst/productPublish!list',
					...
					colModel:[
						...
						{name:'minQuota',index:'minQuota',align:"center",formatter:optionFormatter2, width:70},
						{name:'maxQuota',index:'maxQuota',align:"center",formatter:optionFormatter3, width:70},
						{name:'minTerm',index:'minTerm',align:"center",width:65},
						{name:'maxTerm',index:'maxTerm',align:"center",width:65},
						...
						{name:'reqTotal',index:'reqTotal',align:"center", width:50},
			...
			function optionFormatter(cellvalue, options, rowObject){
				...
				if("上架" == data){
					return '[<a href="JavaScript:viewProduct('+id+');">详情</a>] [<n:instAuthA href="javascript:downStore('+id+');" authCode="WDCP_XJ" value="下架"></n:instAuthA>]' ;
				}else{
					return '[<a href="JavaScript:viewProduct('+id+');">详情</a>] [<n:instAuthA href="javascript:upStore('+id+');" authCode="WDCP_SJ" value="上架"></n:instAuthA>]';
				};
			}
			
			function productAdd(){
				window.location="${ctx}/inst/productPublish!productAdd";
			}
			
			function viewProduct(no){
				window.location.href="${ctx}/inst/productPublish!viewProduct?index="+no;
			}
			
			function downStore(no){
				if(confirm("是否确定下架该借贷产品?")){
					window.location.href="${ctx}/inst/productPublish!downStore?index="+no;
				}
			}
			
			function upStore(no){
				if(confirm("是否确定上架该借贷产品?")){
					window.location.href="${ctx}/inst/productPublish!upStore?index="+no;
				}
			}
			...
			function storeQuery(){
				...
				$("#list").jqGrid('setGridParam',{url:'${ctx}/inst/productPublish!list?yearRate='+yearRate+'&pdtName='+encodeURI(pdtName)+'&pdtStatus='+pdtStatus+'&provinceCode='+provinceCode+'&region='+encodeURI(region)+'&minQuota='+minQuota+'&MaxQuota='+MaxQuota+'&minterm='+minterm+'&Maxterm='+Maxterm+'&MinyearRate='+MinyearRate+'&MaxyearRate='+MaxyearRate+'&fastday='+fastday+'&times='+times+'&rateType='+rateType+'&minTermType='+minTermType+'&maxTermType='+maxTermType,page:1}).trigger("reloadGrid");
			}
		</script>
