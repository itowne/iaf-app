...
<html>
	...
	<body>
		<div id="workframe-wrapper" class="clearfix">
			...
			<div id="content">
				<h3 class="title"><span>修改角色详细信息</span></h3>
				<div class="info" align="right">
						<!--  <input type="button" class="dark-btn" value="导入已有角色编辑" onclick=""/>-->
				</div>
				<div class="results report">
				<s:form id="roleForm" action="#">
					<table id="addRole" width="80%" border="0" cellspacing="0" cellpadding="0" class="data-table">
						<tr>
							<td class="label">角色名称：</td>
							<td class="data"><s:textfield id="roleName" name="roleAuth.roleName" maxLength="30" onblur="check(this);"/><font color=red>*<s:property value="%{roleNameError}"/></font>
							<span id="msg"></span></td>
						</tr>
						<tr>
					...
					<div class="uop">
						<n:instAuthButton cssClass="dark-btn" onclick="checkform()" value="修&nbsp;&nbsp;改" authCode="XTGL_JSQX_XGJS"/>
      					<s:if test="roleAuth.roleStat==1">
							<n:instAuthButton cssClass="dark-btn" onclick="cancel()" value="暂&nbsp;&nbsp;停" authCode="XTGL_JSQX_ZTJS"/>
      					</s:if>
						<s:else>
							<n:instAuthButton cssClass="dark-btn" onclick="recover()" value="启&nbsp;&nbsp;用" authCode="XTGL_JSQX_ZTJS"/>
      					</s:else>
		...
		$('#checkAll').click(function(){
			if($("#checkAll").attr("checked")=="checked"){
				...
				var flag = '${roleAuth.flag}';
				if(flag==1){
					$("#XTGL_JSQX_MENU").attr("checked",true);
					$("#XTGL_ZHGL_MENU").attr("checked",true);
					$("#JSQX_AUTH_ALL :checkbox").attr("checked",true);
					$("#ZHQX_AUTH_ALL :checkbox").attr("checked",true);
					
					$("#JSQX_AUTH_SELECT_ALL").attr("checked",true);
					$("#ZHQX_AUTH_SELECT_ALL").attr("checked",true);
					
					$("#XTGL_JSQX_MENU").attr("disabled",false);
					$("#XTGL_ZHGL_MENU").attr("disabled",false);
					$("#JSQX_AUTH_ALL :checkbox").attr("disabled",false);
					$("#ZHQX_AUTH_ALL :checkbox").attr("disabled",false);
					
					$("#JSQX_AUTH_SELECT_ALL").attr("disabled",false);
					$("#ZHQX_AUTH_SELECT_ALL").attr("disabled",false);
				}else{
					$("#XTGL_JSQX_MENU").attr("disabled",true);
					$("#XTGL_ZHGL_MENU").attr("disabled",true);
					$("#JSQX_AUTH_ALL :checkbox").attr("disabled",true);
					$("#ZHQX_AUTH_ALL :checkbox").attr("disabled",true);
					
					$("#JSQX_AUTH_SELECT_ALL").attr("disabled",true);
					$("#ZHQX_AUTH_SELECT_ALL").attr("disabled",true);
					
				$("#XTGL_JSQX_MENU").attr("checked",false);
				$("#XTGL_ZHGL_MENU").attr("checked",false);
				$("#JSQX_AUTH_ALL :checkbox").attr("checked",false);
				$("#ZHQX_AUTH_ALL :checkbox").attr("checked",false);
				
				$("#JSQX_AUTH_SELECT_ALL").attr("checked",false);
				$("#ZHQX_AUTH_SELECT_ALL").attr("checked",false);
				}
			}else{
	...
	if($("#XTGL_MENU_ALL").attr("checked")=="checked"){
		if($("#XTGL_MENU input[type='checkbox']:checked").length==0){
			...
			return false;
				...
				$("#roleForm").attr("action","${ctx}/inst/instRole!instRoleUpdate");
				$("#roleForm").submit();
			}
			...
			function cancel(){
				if(confirm("暂停角色会导致角色对应的账户不能使用,是否确认要暂停此角色?")){
					window.location.href = "${ctx}/inst/instRole!cancelRole";
				}
			}
			
			function recover(){
				if(confirm("是否启动此角色?")){
					window.location.href = "${ctx}/inst/instRole!recoverRole";
				}
			}
		</script>
