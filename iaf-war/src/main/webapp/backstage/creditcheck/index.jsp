	...
	<body>
		<div id="workframe-wrapper" class="clearfix">
			...
			<div id="content">
				...
				<s:form id="form" action="creditCheck" namespace="backstage">
				...
				<div style="float:right;padding: 0"><n:HcAuthButton cssClass="dark-btn" value="批量下载对账文件" onclick="dow()" authCode="FKDZ_XZ"/></div>
		...
		<script type="text/javascript">
			...
			function getList(){
				jQuery("#list").jqGrid({
					url:'${ctx}/backstage/creditCheck!list',
					colNames:['借贷编号','交易订单号','放贷机构','借款商户','放款金额','放款日期','状态','操作'],
					colModel:[
						{name:'loanOrdId',index:'loanOrdId',align:"center", width:65},
						{name:'otherSysNo',index:'otherSysNo',align:"center", width:70},
						{name:'instName',index:'instName',align:"center", width:50},
						{name:'merchName',index:'merchName',align:"center", width:50},
						{name:'amount',index:'amount',align:"center", width:50},
			...
			function operFormatter(cellvalue, options, rowObject){
				...
				var tmp=rowObject[6];
				if(data=="OTH_CREDIT"||tmp=="已对账"||tmp=="退款"){
					return '[<a href="JavaScript:proc('+index+');">对账详情</a>]&nbsp&nbsp';
					//+ '[<n:HcAuthA authCode="FKDZ_XZ" href="JavaScript:download('+index+');" value="下载订单"></n:HcAuthA>]';
				}else{
			...
			function proc(index){
				parent.jDialog("放款明细","${ctx}/backstage/creditCheck!viewDetail.action?method:viewDetail&index=" + index,860,540,true,function(){
					jQuery("#list").trigger('reloadGrid');
				});
			}
			
			function download(rowId){
				window.location = "${ctx}/backstage/creditCheck!downloadOrd?index=" + rowId;
			}
			
			function dow(){
				document.form.action="${ctx}/backstage/creditCheck!download";	
			...
			function storeQuery(){
				...
				$("#list").jqGrid('setGridParam',{url:'${ctx}/backstage/creditCheck!list?loanOrdId='+loanOrdId+'&otherNo='+otherNo+'&iinst='+inst+'&merchName='+encodeURI(merchName)+'&quota='+quota+'&beginDate='+beginDate+'&fundFlowStat='+fundFlowStat+'&endDate='+endDate+'&maxQuota='+maxQuota,page:1}).trigger("reloadGrid");
			}
