	...
	<body>
				...
				<s:form id="roleForm" action="backstageRole" namespace="/backstage/backstagerole">
					<table width="80%" border="0" cellspacing="0" cellpadding="0" class="data-table">
						<tr>
							...
							<td class="data"><s:textfield id="role_Name" name="bsRoleAuth.roleName" maxLength="30" cssClass="validate[required,minSize[2]]"/><font color=red>*</font>
							</td>
						</tr>
						<!-- 
						<tr>
					...
					<div class="uop">
						<s:submit cssClass="dark-btn" value="修改" method="updateRole" onclick="return doSubmit();"/>
		...
		<script type="text/javascript">
		 <c:forEach items="${authName}" var="authName">
			...
			if($("#SHGL_MENU input[type='checkbox']:checked").length>0){
			...
			if($("#JGGL_MENU input[type='checkbox']:checked").length>0){
			...
			if($("#JGJS_AUTH_ALL input[type='checkbox']:checked").length==$("#JGJS_AUTH_ALL input[type='checkbox']").length){
			...
			if($("#XTCSSZ_MENU input[type='checkbox']:checked").length>0){
